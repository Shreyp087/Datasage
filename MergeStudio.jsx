...
const STRATEGY_LABELS = {
  exact:       "Exact match",
  normalized:  "Normalised (lowercase + trim)",
  numeric:     "Numeric cast",
  date:        "Date comparison",
  fuzzy_substr:"Fuzzy substring",
  id_extract:  "ID extraction (e.g. INC-42 → 42)",
};
    ...
    >
      ...
      >
        ...
        <span>
          ~
          <span style={{ color: C.text, fontWeight: 600 }}>
            {candidate.merged_rows.toLocaleString()}
          </span>{" "}
          output rows
        </span>
      </div>
...
export default function MergeStudio() {
  ...
  useEffect(() => {
    api("/datasets/")
      .then((data) => setDatasets(data.filter((d) => d.status === "complete")))
      .catch(() => {});
  }, []);
  ...
  const handleDetect = useCallback(async () => {
    ...
    setApplyResult(null);
    try {
      const data = await api("/merge/detect", {
        method: "POST",
        body: JSON.stringify({
  ...
  const handleApply = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await api("/merge/apply", {
        method: "POST",
        body: JSON.stringify({
          left_dataset_id: leftId,
          right_dataset_id: rightId,
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "28px 32px" }}>
        ...
        {applyResult && (
          ...
          >
            ...
            >
              <a
                href={`/datasets/${applyResult.output_dataset_id}`}
                style={{
                  background: C.accent,
                  color: "#fff",
                  textDecoration: "none",
